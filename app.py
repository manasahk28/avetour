...
#     # Get user profile details from the current_user object or database query
#     user_profile = {
...
# @app.route('/get_travel_history')
# @login_required
# def get_travel_history():
#     # Fetch travel history details from the database for the current user
#     # Replace the dummy data below with actual data retrieval logic
#     travel_history = [
#         ('Destination 1', 'Amount 1'),
#         ('Destination 2', 'Amount 2'),
#         # Add more travel history entries as needed
#     ]
#     return jsonify(travel_history)
...
def payment():
    if request.method == 'POST':
        ...
        lp, lh, lt, pn = [], [], [], []  # lists for costs and package names

        # Iterate through packages
        for p in pkg:
            lp.append(p.estimated_cost)
            pn.append(p.package_name)
        ...
        if lh:  # Ensure lh is not empty
            i = len(lh) - 1
            amt1 = int(lp[i]) if i < len(lp) else 0
            amt2 = int(lh[i])
            amt3 = int(lt[i]) if i < len(lt) else 0
            p = pn[i] if i < len(pn) else ''
            totl = amt1 + amt2 + amt3
            ...
            try:
                ...
                flash(f'There was a problem in making payment: {str(e)}')
                return render_template('payment.html')
        else:
            flash('No hotel records found.')
            return render_template('payment.html')
    else:
        pkg = db.session.query(Package).filter(Package.email == current_user.email).all()
        hotl = db.session.query(Hotel).filter(Hotel.email == current_user.email).all()
...
def delete(id):
    if 'user' in session and session['user'] == 'Admin':
        ...
        try:
            ...
            db.session.commit()
            flash('Package Info successfully Deleted', 'success')
        except SQLAlchemyError as e:
            flash(f'Error deleting package info: {str(e)}', 'error')
...
def deleteT(id):
    if 'user' in session and session['user'] == 'Admin':
        ...
        try:
            ...
            db.session.commit()
            flash('Transportation info successfully deleted.', 'success')
        except SQLAlchemyError as e:
            db.session.rollback()  # Rollback the session in case of error
            flash(f'Error deleting transportation info: {str(e)}', 'error')
        return redirect('/dashboard')
    else:
        flash('You are not authorized to perform this action.', 'error')
...
def deleteC(id):
    if 'user' in session and session['user'] == 'Admin':
        ...
        try:
            ...
            db.session.commit()
            flash('Contact entry successfully deleted', 'success')
        except SQLAlchemyError as e:
            flash(f'Error deleting contact entry: {str(e)}', 'error')
        return redirect('/dashboard')
    else:
        flash('You are not authorized to perform this action.', 'error')
        return redirect('/login')  # Redirect to login page or any other appropriate page
...
def deletePm(id):
    if 'user' in session and session['user'] == 'Admin':
        ...
        try:
            ...
            db.session.commit()
            flash('Payment transaction successfully deleted', 'success')
        except SQLAlchemyError as e:
            flash(f'Error deleting payment transaction: {str(e)}', 'error')
        return redirect('/dashboard')
    else:
        flash('You are not authorized to perform this action.', 'error')
        return redirect('/login')  # Redirect to login page or any other appropriate page
...
def deleteH(id):
    if 'user' in session and session['user'] == 'Admin':
        ...
        try:
            ...
            db.session.commit()
            flash('Accommodation info successfully deleted', 'success')
        except SQLAlchemyError as e:
            flash(f'Error deleting accommodation info: {str(e)}', 'error')
        return redirect('/dashboard')
    else:
        flash('You are not authorized to perform this action.', 'error')
        return redirect('/login')  # Redirect to login page or any other appropriate page
